...
// Actualizar la propiedad `lastActivityTime` del usuario conectado
for (let x of users) {
...
// Función para ordenar los usuarios por `lastActivityTime` en orden descendente
users.sort((a, b) => new Date(b.lastActivityTime) - new Date(a.lastActivityTime));

// Renderizar los usuarios en la lista
function renderUsersList() {
  usersListShow.innerHTML = ""; // Limpiar la lista

  // Recorrer los usuarios para crear los elementos de lista
  for (let x of users) {
    if (signedInUserID != x.userId) {
      var newLiElement = document.createElement("li");

      // Convertir la propiedad `lastActivityTime` a tipo Date
      ...
      newLiElement.innerHTML = `
        <div class="p-0 m-0 d-flex justify-content-between align-items-center">
          <div class="d-flex justify-content-start align-items-center py-2" style="max-width:85%">
              <div class="bg-primary users-pic-div">${x.firstName[0] + x.lastName[0]}</div>
              <p class="p-0 m-0 fw-semibold align-self-center ms-2 fs-4 profile-pic-name" style="width: 75%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${x.firstName + " " + x.lastName}</p>
          </div>
          <div class="d-flex justify-content-center align-items-center">
            <p class="p-0 m-0 fw-light fs-6 text-muted align-self-center me-3">
              ${x.lastActivityTime.getHours().toString().padStart(2, "0") + ":" + x.lastActivityTime.getMinutes().toString().padStart(2, "0")}
            </p>
          </div>
        </div>
      `;
      newLiElement.dataset.chatterInfo = x.userId;
      newLiElement.classList.add("border", "border-3", "pt-1", "ps-1", "user-li-item");

      usersListShow.appendChild(newLiElement); // Agregar el usuario a la lista
    }
  }
}

// Llamar a la función para renderizar la lista de usuarios al cargar
renderUsersList();
      ...
      if (currentChatter == x.userId) {
        ...
        nameDiv.innerHTML = `
          <div class="d-flex justify-content-start align-items-center py-1">
              <div class="bg-primary users-pic-div">${x.firstName[0] + x.lastName[0]}</div>
              <p class="p-0 m-0 fw-bold align-self-center ms-2 fs-4 profile-pic-name">${x.firstName + " " + x.lastName}</p>
          </div>
          <div>
        ...
        `;
        nameDiv.classList.add("px-1", "d-flex", "justify-content-between", "align-items-center");
    ...
    if (signedInUserID == x.userId) {
      for (let messages of x.messageHistory) {
        if (
          ...
          msgDiv.classList.add("px-1", "d-flex", "align-items-center", "w-100", "container-div");
          if (_dum == 0) {
          ...
          messages.messageContent.messageTime = new Date(messages.messageContent.messageTime);
          msgDiv.dataset.timesent = messages.messageContent.messageTime.toISOString();
          msgDiv.innerHTML = `
            <div class="py-1 msg-container-div rounded rounded-2 ps-2 pe-3" style="max-width:60%;">
                <p style="white-space:pre-wrap" class="text-start d-inline p-0 m-0 fw-semibold align-self-center text-white text-break">${messages.messageContent.messageText}</p>
                ...
                ${messages.messageContent.messageTime.getHours().toString().padStart(2, "0") + ":" + messages.messageContent.messageTime.getMinutes().toString().padStart(2, "0")}
                </p>
    ...
    if (signedInUserID == x.userId) {
      x.messageHistory = x.messageHistory.filter(
        (messageContentElement) =>
          new Date(messageContentElement.messageContent.messageTime).toISOString() != new Date(msgTimeSent).toISOString()
      );
    }
  }
  localStorage.setItem("usersLocalStorage", JSON.stringify(users));
...
function deleteAllMessages() {
  for (let x of users) {
    if (signedInUserID == x.userId) {
      x.messageHistory = x.messageHistory.filter(
        (messageContentElement) => messageContentElement.chattingID != currentChatter
      );
    }
  }
...
function searchFunc() {
  ...
  if (searchInput != "") {
    let searchStrings = searchInput.split(" ").filter((str) => str.trim() !== "");
    let toSearch = "";
    ...
    for (let x of users) {
      if (signedInUserID != x.userId) {
        ...
        if (
          ...
          toSearch == x.firstName.toLowerCase() + x.lastName.substring(0, searchInput.length - x.firstName.length - 1).toLowerCase()
        ) {
